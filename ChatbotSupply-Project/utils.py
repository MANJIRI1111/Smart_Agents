...
def get_closest_match(input_name, options, threshold=80):
    match, score = process.extractOne(input_name, options)
    if score >= threshold:
        return match
    return None
...
def Ret_Qty_from_DB(ItemName):
    ...
    DB_Cursor.execute("SELECT ProductName FROM Products")
    product_names = [row[0] for row in DB_Cursor.fetchall()]

    # Use fuzzy matching to find the closest product name
    closest_match = get_closest_match(ItemName, product_names)
    if not closest_match:
        return f"No matching product found for '{ItemName}'. Please check the product name."

    print(f"Closest match for '{ItemName}': {closest_match}")

    # Query to retrieve quantity based on the matched name
    cmdstring = '''Select 
                   sum(case when t.stockin_stockout = FALSE then t.quantitySold else 0 end) - 
                   sum(case when t.stockin_stockout = TRUE then t.quantitySold else 0 end) as 'Quantity'
                   from Products p left join Transaction_Db t
                   on p.ProductID = t.ProductID
                   where p.ProductName = %s or p.ProductID = %s'''
    val = (closest_match, closest_match)
    ...
    if result and result[0] is not None:
        x = result[0]
        print(f"Quantity for '{closest_match}': {x}")
        return str(x)
    else:
        return f"Could not retrieve quantity for '{closest_match}'."
